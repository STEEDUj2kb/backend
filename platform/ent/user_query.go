...
func (uq *UserQuery) QueryStudies() *StudyQuery {
	query := &StudyQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withStudies: uq.withStudies.Clone(),
...
func (uq *UserQuery) WithStudies(opts ...func(*StudyQuery)) *UserQuery {
	query := &StudyQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withStudies = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withStudies; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Studies = []*Study{}
		}
		query.withFKs = true
		query.Where(predicate.Study(func(s *sql.Selector) {
			s.Where(sql.InValues(user.StudiesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_studies
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_studies" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_studies" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Studies = append(node.Edges.Studies, n)
		}
	}
